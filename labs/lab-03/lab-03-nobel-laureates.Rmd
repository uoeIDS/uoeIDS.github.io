...
-   **Task 2:** Read about merge conflicts below. The merge conflict exercise we'll start with during the workshop will assume that you have this background information.

## Merges and merge conflicts

You should by now have a bit of experience with Git and Github, and so now lets deliberately make a merge conflict so we can have more practice fixing them.
...
**EVERYONE:** Go to course [GitHub organization](https://github.com/ids-s1-21) and locate your Lab 03 repo, which should be named `lab-03-nobel-laureates-YOUR_TEAMNAME`.
...
```{marginfigure}
Note that your bar plot won't exactly match the one from the BuzzFeed News article. This is likely because the data has been updated since the article was published.  Don't worry if you can't make the bar plot have all the bars in decreasing order of size---but you might like to try anyway!
```

6.  In a single pipeline, filter for laureates who won their prize in the US, but were born outside of the US, and then create a frequency table (with the `count()` function) for their birth country (`born_country`) and arrange the resulting data frame in descending order of number of observations for each country. **Note that we're now no longer restricting to only the four science prizes.** Which country is the most common? Make a (horizontal) bar plot of the data.

```{marginfigure}
Hint: you'll need to go back to the `nobel_living` data frame.  You may wish to use `mutate` again to recreate the `country_us` and `born_country_us` variables.
```
