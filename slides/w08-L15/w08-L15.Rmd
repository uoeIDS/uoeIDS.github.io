...
???
linear models are actually very useful in real life, often prefered to more complicated models
because they are simple, fast, easy to interpret, they work pretty well most of the times
...
???
It's a dataset coming from an auction catalogue, it's made up of 28 auction sales in Paris in ...
These are paper printed catalogues that people would read and decide what they wanted to bid on.
Was transcribed by two PhD students in Duke, took these paper records and translated and tabulated into an electronic dataset.
...
```{r ref.label = "height-width-plot-01", echo = FALSE, warning = FALSE, out.width = "60%"}
```
]
...
???
here is the representation of the relationship between those two variables
and the blue line is basically the model that we are fitting to quantify the relationship 
- positive slope (if the painiting is getting bigger in width, it's also getting bigger in height)
- code on how drawing the best fit line (discarding NAs)
- notice the gray band around the blue line: there is uncertainty around our model 
because we do not have data on all the possible paintings, we have a sample
- the larger the band, the larger the uncertainty
...
.panelset[
.panel[.panel-name[Plot]
```{r ref.label = "height-width-plot-gam", echo = FALSE, warning = FALSE, out.width = "50%"}
```
]
.panel[.panel-name[Code]
...
???
Generalized additive models are a kind of linear regression, but instead of finding coefficients of predictor variables (e.g., intercepts, slopes), the model finds a “smooth” response function for each predictor. Tipically, a piecewise cubic function (spline)
...
.panelset[
.panel[.panel-name[Plot]
```{r ref.label = "height-width-plot-loess", echo = FALSE, warning = FALSE, out.width = "50%"}
```
]
.panel[.panel-name[Code]
...
LOESS smooths are constructed by making a large number of quadratic (or possibly linear) regression lines as a window moves along the x-axis. The predictions from these many local regressions are then computed and plotted as a “smooth” of the data. 
...
```{r height-width-plot-02,echo=FALSE, warning = FALSE, out.width = "40%"}
ggplot(data = pp, aes(x = Width_in, y = Height_in)) +
  geom_point() +
  geom_smooth(method = "lm",se=FALSE) +
  labs(
...
## Portrait vs landscape paintings

```{r out.width="50%", echo=FALSE}
knitr::include_graphics("img/height-width-plot-alpha-portrait.png")
```
...
```{r out.width="50%", echo=FALSE}
knitr::include_graphics("img/height-width-plot-alpha-landscape.png")
```
...
???
Aim of tidymodels: make the modeling process simple and logical, and give a unified 
framework for fitting models
