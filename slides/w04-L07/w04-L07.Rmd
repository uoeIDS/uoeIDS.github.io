...
```{r location, echo=FALSE, fig.height=6}
stats <- tibble(style = c("Left Skew", "Symmetric","Right Skew"),
       shape1 = c(2,5,5), shape2 = c(5,5,2) ) %>%
  mutate(avg = shape1/(shape1+shape2), med=qbeta(0.5,shape1,shape2),
         mode = (shape1-1)/(shape1+shape2-2)) %>%
    mutate(style = fct_relevel(style,"Left Skew","Symmetric", "Right Skew"))

vals <- tibble(x=seq(0,1,by=0.01)) %>%
    mutate(
      `Left Skew` = dbeta(x, stats$shape1[1], stats$shape2[1]),
      `Symmetric` = dbeta(x, stats$shape1[2], stats$shape2[2]),
      `Right Skew` = dbeta(x, stats$shape1[3], stats$shape2[3])
      ) %>%
    pivot_longer(cols = c("Left Skew","Symmetric", "Right Skew"), names_to = "style", values_to = "y") %>%
    mutate(style = fct_relevel(style,"Left Skew","Symmetric", "Right Skew"))
