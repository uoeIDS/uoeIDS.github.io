...
.small[
"These are not the droids you're looking for!"<img style='height:18px' src='img/C3PO.webp'><img style='vertical-align:middle;' src='img/R2D2.webp'>
]
...
.pull-left[
`r emo::ji("white_check_mark")` Informative variable name
...
.pull-right[
`r emo::ji("x")` Bad/sloppy variable name
...
```{r bad_add, eval=FALSE}
starwars +
  select(name, height)
```
```{r bad_add1, echo=FALSE}
cat("Error in select(name, height) : object \n'name' not found")
```

`r emo::ji("white_check_mark")`

```{r good_pipe, output.lines = 6}
starwars %>%
  select(name, height)
```
]
