...
- Automate common tasks in a more powerful and general way than copy-and-pasting:
  - Give your function an informative name that makes your code easier to understand
...
area <- function(r) {
  if (r < 0) 
    stop("Error, provide positive radius.")
  return(pi*r^2)
}
area(2)
```
...
```{r eval=FALSE}
area(-2)  
# Error in area(-2) : Error, provide positive radius.
```
---

Let's try a different input, say with a logic value.
```{r}
area(TRUE)
```
...
```{r eval=FALSE}
area(TRUE)  
# Error in area(TRUE) : Error, provide positive numeric radius.
```
