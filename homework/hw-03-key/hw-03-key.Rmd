...
```{r echo = FALSE}
coefs <- linear_fit %>% tidy() %>% pull(estimate)
```


Formula of the fitted model (roughly): 
$\widehat{emailhr} =$ `r round(coefs[1],2)` + `r round(coefs[2],2)` $\times \textrm{educ}$.

The coefficient for `emailhr` explains that people with an additional year in education are *expected* to spend **on average** `r round(coefs[2],2)` hours more on email per week.
...
```{r echo=FALSE}
R2 <- glance(linear_fit) %>% pull(r.squared)
```


When the simple linear regression is considered for the `emailhr` variable, the model results in poor/moderate performance. 
In fact, the model R-squared value is very close to the zero (`r round(R2,3)`), and this shows that these variables are not able to explain a large variation in the response variable -- only `r round(100*R2,1)`% of the variation in `emailhr` is explained by the model.
...
Nevertheless from this scatterplot we can deduce that something is off (the increasing variance of the residual around the fitted line). 
...
```{r echo=FALSE}
AUC1 <- gss16_test_pred_1 %>%
  roc_auc(truth = advfront, .pred_Agree, event_level = "second") %>%
  pull(.estimate)
```


This model has a AUC (areal under the curve) of `r round(AUC1,2)`. 
...
```{r}
gss16_rec_2 <- recipe(advfront ~ educ + wrkstat + polviews + emailhr,
                      data = gss16_train) %>%
  step_dummy(all_nominal(), -all_outcomes())
...
AUC2 <- gss16_test_pred_2 %>%
  roc_auc(truth = advfront, .pred_Agree, event_level = "second") %>%
  pull(.estimate)
```

The area under the ROC curve for the second model (`r AUC2`) is moderately less than the first model (`r AUC1`). 

#### Exercise 4 (c)

Model 1 has better AUC (area under ROC curve) than model 2. Moreover, model 1 has fewer variables, which is generally preferred due to reduced model complexity (Occam's razor), run time, and better generalizability, so even if they have comparable preformance, this model should be favoured.
