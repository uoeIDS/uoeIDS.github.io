...
In this assignment you will work on data collection and data cleaning.
...
## Exercise 1
Create a single data set and called it `pac_all` containing the constributions in all election years available. In your R Markdown file, load `pac_all.csv` and report its number of observations and variables as a full sentence.
...
```{marginfigure}
__Note:__ If you can't get the merge working, a `pac_all_backup.csv` already exists in your repository for you to work on.
```
...
ggplot(data = mpg, aes(x = cty, y = hwy, color = drv)) +
  geom_point(alpha=10)+theme(plot.background = element_rect(fill = "green"),axis.line = element_line(size = rel(6), arrow = arrow(), color = "black"))+theme(panel.background = element_rect(fill = "red", colour = "grey50"))+  theme(axis.title.y = element_text(size = rel(2.0), angle = 99))+  theme(axis.title.x = element_text(size = rel(.6), angle = 40)) +theme(panel.grid.major = element_line(colour = "green"))


```

```{marginfigure}
I realize that "pretty" is subjective, so we're mostly looking to see if you can figure out how to change the look of a plot using help files of functions you haven't learned before.
```

## Exercise 5
Make this plot as pretty as possible by changing colours, background color, fonts, or anything else you can think of. You will probably want to play around with [theme options](https://ggplot2.tidyverse.org/reference/theme.html), but you can do more. You can also search online for other themes, fonts, etc. that you want to tweak. Try to make it as pretty as possible, the sky is the limit!
