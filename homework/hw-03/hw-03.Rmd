...
a) Fit a linear regression model to predicting `emailhr` based on the `educ`. From your output, state the formula for the line-of-best fit and give an interpretation of the `emailhr` estimate. (3 pts)

b) Comment on the overall performance of the linear regression model. Support your statements with an appropriate data visualisation and model fit statistics. (2 pts)
...
a) Build a workflow for the training data that consists of a recipe (`gss16_rec_1`) and a model (`gss16_mod_1`). Name this workflow `gss16_wflow_1`. (4 pts)
...
a) Apply the workflow you defined earlier to the training dataset and named the model as `gss16_fit_1`. Display the resulting tibble containing the fitted model parameters. (2 pts)

b) Use the fitted models to predict the test data, plot the ROC curve for the predictions. (3 pts)
...
Then use the fitted models to predict the test data, plot the ROC curve for the predictions for both models, and calculate the areas under the ROC curves. (3 pts)
