...
subtitle: "Due: 21 November, 16:00 UK time"
output: 
  tufte::tufte_html:
    css: ../hw.css
...
NOTE: for this homework, you do <u>not</u> need to add the course GitHub account as a collaborator. Only in case of technical problems with your submission, we might reach out to ask that you add the course GitHub account as a collaborator to your homework repository. You do <u>not</u> have to do this as a default (invites sent before the submission deadline will not be monitored).
...
a) Fit a linear regression model to predict `emailhr` based on the `educ`. From your output, state the formula for the line-of-best fit and give an interpretation of the `emailhr` estimate. (2 pts)
...
```{marginfigure}
Using a cutoff of 0.83 means that the model predicts $\hat{y}_i$ to be "Agree" (or `TRUE`, or 1) when $\hat{p}_i \geq 0.83$ and $\hat{y}_i$ equal to "Not agree" when $\hat{p}_i$ is less than 0.83.
```

b) Produce a table with the number of true positives, true negatives, false positives and false negatives on the *test data* when using a cutoff of 0.83 for the predictions.
Then calculate the specificity and the sensitivity of the model (using the same cutoff). (2 pts)
...
We are now going to model `advfront` using the explanatory variables `polviews`, `wrkstat`, `emailhr`, and `educ`.
...
Now the new recipe (named `gss16_rec_2`) should contain an appropriate formula for predicting `advfront` from `polviews`, `wrkstat`, `emailhr`, and `educ`, and should use:
...
```{marginfigure}
Mainly, this exercise is an extension to the earlier model by adding `polviews`, `emailhr` and `wrkstat` as extra explanatory variables.
```
...
c) Comment on which model performs better 
  ...
  * the model including `polviews`, `wrkstat`, `emailhr`, and `educ` with `gss16_split` as model 2 (`gss16_fit_2`)
...
3.    Click on the **Dashboard** in the left-hand panel and select **Homework C (hw-03)**.
